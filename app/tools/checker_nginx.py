...
def send_and_logging(mes, service_status, service_ip, alert_type):
    ...
    try:
        alerting.alert_routing(service_ip, 2, group, level, mes, alert_type)
...
def main(serv, port):
    ...
    while True:
        if first_run:
            service_id = sql.select_service_id_by_slug('nginx')
        ...
        with closing(socket.socket(socket.AF_INET, socket.SOCK_STREAM)) as sock:
            ...
            try:
                if sock.connect_ex((serv, port)) == 0:
                    ...
                    if old_stat != 1:
                        status = 'UP'
                        alert = "NGINX service is " + status + " at " + serv
                    ...
                    if old_stat != 0:
                        status = 'DOWN'
                        alert = "NGINX service is " + status + " at " + serv
                        send_and_logging(alert, status, serv)
    ...
    logging.info(f'NGINX worker has been shutdown for: {serv}')


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Check NGINX servers state.', prog='checker_nginx.py', formatter_class=argparse.ArgumentDefaultsHelpFormatter)

    parser.add_argument('IP', help='Start check NGINX server state at this ip', nargs='?', type=str)
    parser.add_argument('--port', help='Start check NGINX server state at this port', nargs='?', default=8086, type=int)
